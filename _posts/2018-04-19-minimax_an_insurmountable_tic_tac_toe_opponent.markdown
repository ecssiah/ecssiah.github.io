...
It may seem unnecessary to take this into account, since the minimax AI will alway play perfectly, so it will never get into a situation where it will face a loss. But it's entirely possible you might want to introduce a little randomness into the AI so that it is actually possible for a human to win sometimes. Generally, players do not enjoy a game that just ruthlessly beats them every time, so an AI that intentionally makes the occasional mistake could actually be a better design.
